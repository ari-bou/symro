...
class MetaSet(MetaEntity):
    ...
    def __init__(self,
        ...
        self._set_node: BaseSetNode = set_node
    ...
    def get_set_node(self) -> BaseSetNode:
        if self._parent is None:
            return self._set_node
        else:
            return self._parent.get_set_node()

    def get_expression_nodes(self) -> List[ExpressionNode]:
        expr_nodes = []
        if self.idx_set_node is not None:
            expr_nodes.append(self.idx_set_node)
        if self._super_set_node is not None:
            expr_nodes.append(self.get_super_set_node())
        if self._set_node is not None:
            expr_nodes.append(self.get_set_node())
        return expr_nodes
    ...
    def generate_declaration(self) -> str:
        ...
        set_node = self.get_set_node()
        if set_node is not None:
            declaration += " := {0}".format(set_node)
