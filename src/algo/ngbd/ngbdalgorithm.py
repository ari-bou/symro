...
from symro.src.algo.ngbd.ngbdproblembuilder import NGBDProblemBuilder
...
class NGBDAlgorithm(GBDAlgorithm):
    ...
    def __init__(self,
        ...
        # --- Callables ---
        self.before_nc_sp_solve: Callable[[GBDProblem, int, str, mat.Element], None] = before_nc_sp_solve
    ...
    def _check_outer_loop_termination_criteria(self, ub_nc: float) -> bool:
        ...
        if epsilon_rel <= self.rel_opt_tol:  # relative error within tolerance
            message = f"NGBD epsilon-optimal solution with relative error = {epsilon_rel}"
        ...
        if self.abs_opt_tol is not None:
            ...
            if epsilon_abs <= self.abs_opt_tol:  # absolute error within tolerance
                message = f"NGBD epsilon-optimal solution with absolute error = {epsilon_abs}"
                self._log_indexed_message(message, self.it)
                return False
    ...
    def __solve_nc_primal_sp(
            self,
            nc_sp_container: GBDSubproblemContainer,
        ...
        self._nc_engine.set_active_problem(
            problem_symbol=sp_sym,
            problem_idx=sp_idx
        )
        ...
        self.__fix_origin_complicating_variables(
            ...
            sp_container=nc_sp_container,
            y=y
        )

        if self.before_nc_sp_solve is not None:
            self.before_nc_sp_solve(self.gbd_problem,
                                    self.it,
                                    sp_sym,
                                    sp_idx)
