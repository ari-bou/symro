...
from symro.src.algo.gbd import *
from symro.src.algo.ngbd.ngbdproblem import NGBDProblem
...
class NGBDProblemBuilder(GBDProblemBuilder):
    ...
    def build_or_retrieve_defined_primal_sp(self,
        ...
        if entity_defs is None:
            ...
            if sp_sym in self.origin_problem.subproblems:  # subproblem is defined in the original problem
                origin_sp = self.origin_problem.subproblems[sp_sym]
                self.gbd_problem.origin_primal_sps.append(origin_sp)  # store original primal subproblem

            else:  # subproblem is not defined in the original problem
                raise ValueError("NGBD problem builder encountered an undefined subproblem symbol")

            if sp_sym in self.gbd_problem.subproblems:  # subproblem is defined in the convex relaxation
        ...
        # build new subproblem
            ...
            origin_sp = BaseProblem(symbol=sp_sym, description="Original primal subproblem")
            ...
            convex_sp = BaseProblem(symbol=sp_sym, description="Convexified primal subproblem")
    ...
    def _build_default_original_primal_sp(self):
        ...
        if primal_sp_meta_obj is None:
            raise ValueError("NGBD problem builder could not identify a suitable objective function"
    ...
    def __build_original_subproblem_containers(self):
        ...
        for sp_idx_set_dict, nc_sp_container in zip(sp_idx_sets, self.ngbd_problem.nc_sp_containers):
    ...
    def __build_model_script_origin(self):
        ...
        ampl_stm.add_included_script_to_compound_script(
            self.origin_problem.compound_script,
            mod_script,
            file_command="model"
        )
