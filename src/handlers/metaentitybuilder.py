...
def build_meta_set(problem: Problem,
                   symbol: str,
                   alias: str = None,
    ...
    if idx_set_node is None:
        idx_set_node = nb.build_idx_set_node(problem, idx_meta_sets, idx_set_con_literal)
...
def build_meta_param(problem: Problem,
                     symbol: str,
                     alias: str = None,
                     idx_meta_sets: Union[List[mat.MetaSet], Dict[str, mat.MetaSet]] = None,
                     idx_set_con_literal: str = None,
    ...
    if idx_meta_sets is None and idx_set_node is not None:
        idx_meta_sets = _build_idx_meta_sets_of_meta_entity(
            problem=problem,
            idx_set_node=idx_set_node,
            expr_nodes=[defined_value_node, default_value_node, super_set_node] + list(relational_constraints.values()))
...
def build_meta_var(problem: Problem,
                   symbol: str,
                   alias: str = None,
                   idx_meta_sets: Union[List[mat.MetaSet], Dict[str, mat.MetaSet]] = None,
                   idx_set_con_literal: str = None,
    ...
    if idx_meta_sets is None and idx_set_node is not None:
        idx_meta_sets = _build_idx_meta_sets_of_meta_entity(
            problem=problem,
            idx_set_node=idx_set_node,
            expr_nodes=[defined_value_node, default_value_node, lower_bound, upper_bound])
...
def build_meta_obj(problem: Problem,
                   symbol: str,
                   alias: str = None,
                   idx_meta_sets: Union[List[mat.MetaSet], Dict[str, mat.MetaSet]] = None,
                   idx_set_con_literal: str = None,
...
def build_meta_con(problem: Problem,
                   symbol: str,
                   alias: str = None,
                   idx_meta_sets: Union[List[mat.MetaSet], Dict[str, mat.MetaSet]] = None,
                   idx_set_con_literal: str = None,
...
def _build_value_node(value: Union[None, int, float, str, mat.ExpressionNode]
    ...
    if value is not None:
        ...
        if isinstance(value, int) or isinstance(value, float):
            ...
            node = mat.StringNode(literal=value,
                                  delimiter='"')
    return node
...
def _build_idx_meta_sets_of_problem(problem: Problem, subproblem: BaseProblem = None):
    ...
    for me_iterable in (subproblem.model_meta_sets_params,
...
def build_idx_meta_sets(problem: Problem,
                        idx_set_node: mat.CompoundSetNode,
...
def __build_sub_meta_entity_idx_set_node(problem: Problem,
                                         idx_subset_node: mat.CompoundSetNode,
                                         meta_entity: mat.MetaEntity,
                                         entity_idx_node: mat.CompoundDummyNode):
...
def __build_subset_membership_node(problem: Problem,
    ...
    set_node_map: Dict[int, mat.SetExpressionNode] = {id(node): node for node in idx_subset_node.set_nodes}
    set_node_positions = {id(node): pos for pos, node in enumerate(idx_subset_node.set_nodes)}
    ...
    (def_unb_syms,
     ...
     inner_set_node_ids,
     middle_unb_syms,
     inner_unb_syms) = __assign_component_set_nodes_to_inner_and_middle_scopes(
        idx_subset_node,
        entity_idx_node)
    ...
    set_node = __build_set_node(
        middle_idx_set_node,
        inner_idx_set_node)

    set_member_node = __build_set_member_node(
        inner_to_outer_map,
        inner_unb_syms)

    set_membership_node = mat.SetMembershipOperationNode(operator="in",
            ...
            # compound dummy node
                ...
                for component_dummy_node in dummy_node.component_nodes:
                    if isinstance(component_dummy_node, mat.DummyNode):
                        scope = handle_dummy_node(id(component_set_node), component_dummy_node)
                        if scope == 1:
                            is_middle = True
                        elif scope == 0:
                            is_inner = True
...
def __build_middle_scope_idx_set_node(problem: Problem,
    ...
    if len(middle_set_node_ids) > 0:
        ...
        while not queue.empty():
            ...
            unbound_syms = nb.retrieve_unbound_symbols(set_node_map[node_id], in_filter=def_unb_syms)
...
def __build_inner_scope_idx_set_node(problem: Problem,
                                     idx_subset_node: mat.CompoundSetNode,
                                     inner_idx_set_con_ops: List[mat.LogicalExpressionNode],
                                     set_node_map: Dict[int, mat.SetExpressionNode],
                                     set_node_positions: Dict[int, int],
    ...
    if len(inner_set_node_ids) == 0:
        ...
        inner_set_nodes = [mat.OrderedSetNode(start_node=one_node,
                                              end_node=one_node)]
    else:
    ...
    if len(inner_idx_set_con_ops) == 1:
        ...
        inner_idx_set_node.constraint_node = nb.build_conjunction_node(inner_idx_set_con_ops)

    return inner_idx_set_node


def __build_set_node(middle_idx_set_node: mat.CompoundSetNode,
