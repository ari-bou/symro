...
from symro.parsing import outputparser

from symro.algo.gbd import GBDProblem, GBDSubproblemContainer, GBDProblemBuilder
...
class GBDAlgorithm:
    ...
    def add_decomposition_subproblem(
        self,
        sp_symbol: str,
        entity_defs: List[str] = None,
        linked_entity_defs: List[str] = None,
    ):
        self._gbd_problem_builder.build_or_retrieve_defined_primal_sp(
            sp_sym=sp_symbol,
    ...
    def _run_algorithm(self) -> Tuple[float, EntityValueCollection]:
        ...
        if is_feasible:
            self.__log_message(
                f"{self.algorithm_name} algorithm terminated with feasible solution"
            )
        else:
            self.__log_message(
                f"{self.algorithm_name} algorithm terminated with infeasible solution"
            )
    ...
    def _run_loop(
        ...
        while can_iterate:
            ...
            if can_iterate:
                ...
                self.__set_is_feasible_flag(
                    True
                )  # set default value of feasibility flag
                ...
                self._log_indexed_message(
                    "Primal objective is {0}".format(ub_i), self.it
                )

                self.__set_stored_obj_value(
                    ub_i
                )  # store upper bound of current iteration
            ...
            # output
            self.__print_iteration_output(self.it, self.lb, self.ub, is_it_feasible)
    ...
    def _check_termination_criteria(self) -> bool:
        ...
        if epsilon_rel <= self.rel_opt_tol:  # relative error within tolerance
            message = (
                f"GBD epsilon-optimal solution with relative error = {epsilon_rel}"
            )
        ...
        if self.abs_opt_tol is not None:
            ...
            if epsilon_abs <= self.abs_opt_tol:  # absolute error within tolerance
                message = (
                    f"GBD epsilon-optimal solution with absolute error = {epsilon_abs}"
                )
                self._log_indexed_message(message, self.it)
                return False
    ...
    def __solve_primal_problem(
        self,
        dual_soln: Dict[
            str, Union[float, Dict[Tuple[Union[int, float, str, None], ...], float]]
        ],
    ):
        ...
        for sp_container in self.gbd_problem.sp_containers:
            ...
            if is_feasible_i:
                ...
                is_primal_sp_fbl, v_sp = self.__solve_primal_sp(
                    sp_container=sp_container, dual_soln=dual_soln
                )
                ...
                if is_primal_sp_fbl:
                    ...
                    self._log_indexed_message(
                        "Primal subproblem is infeasible",
                        iter=self.it,
            ...
            if not is_feasible_i:
                _, v_fbl_sp = self.__solve_fbl_sp(
                    sp_container=sp_container, dual_soln=dual_soln
                )
    ...
    def __solve_primal_sp(
        ...
        self._engine.set_active_problem(
            problem_symbol=sp_container.primal_sp.symbol,
        ...
        v_sp = self.__store_sp_result(
            is_feasible=True, sp_container=sp_container, dual_soln=dual_soln
        )
        self._log_indexed_message(
            "Subproblem objective is {0}".format(v_sp),
            iter=self.it,
            sp_sym=sp_container.primal_sp.symbol,
            sp_idx=sp_container.sp_index,
        )
    ...
    def __solve_fbl_sp(
        ...
        self._engine.set_active_problem(
            problem_symbol=sp_container.fbl_sp.symbol, problem_idx=sp_container.sp_index
        )
        ...
        if not self._interpret_solver_result(
            ...
            self._log_indexed_message(
                "Feasibility subproblem is infeasible",
                iter=self.it,
                sp_sym=sp_container.fbl_sp.symbol,
                sp_idx=sp_container.sp_index,
            )
        ...
        v_sp = self.__store_sp_result(
            is_feasible=False, sp_container=sp_container, dual_soln=dual_soln
        )
        self._log_indexed_message(
            "Subproblem objective is {0}".format(v_sp),
            iter=self.it,
    ...
    def _interpret_solver_result(
        self, solver_output: str, sp_sym: str, sp_index: mat.Element
    ) -> bool:
        ...
        if self.sp_solve_options.get("solver", None) == "conopt":
            ...
            # Last iteration yielded an infeasible solution
                ...
                (is_feasible, _, solver_iter) = outputparser.parse_conopt_output(
                    solver_output
                )
                ...
                # Problem is feasible
                    ...
                    self._log_indexed_message(
                        "Resolving feasible problem declared infeasible (CONOPT)",
                        iter=self.it,
                        sp_sym=sp_sym,
                        sp_idx=sp_index,
                    )
                    ...
                    if not is_feasible:
                        raise ValueError(
                            "Failed to recover last feasible solution of a feasible subproblem."
                        )

                    self._log_indexed_message(
                        "Recovered feasible solution (CONOPT)",
                        iter=self.it,
                        sp_sym=sp_sym,
                        sp_idx=sp_index,
                    )
    ...
    def __store_sp_result(
        ...
        self.__retrieve_sp_dual_solution(
            is_feasible=is_feasible, sp_container=sp_container, dual_soln=dual_soln
        )
    ...
    def __store_complicating_variables(
        self,
        ...
        for _, comp_meta_var in self.gbd_problem.comp_meta_vars.items():
            ...
            # Indexed variable
                ...
                var_idx_set = comp_meta_var.idx_set_node.evaluate(
                ...
                for var_idx in var_idx_set:
                    ...
                    value = modify_value(value, comp_meta_var)
                    self._engine.set_param_value(
                        storage_sym, var_idx + (cut_count,), value
                    )
                    y_var[var_idx] = value

        return y

    def _fix_complicating_variables(
        self, engine: Engine, sp_container: GBDSubproblemContainer
    ):
        ...
        for var_sym, idx_set in sp_container.comp_var_idx_sets.items():
            ...
            # indexed variable
                ...
                for var_index in idx_set:
                    value = engine.get_param_value(
                        storage_sym, var_index + (cut_count,)
                    )
    ...
    def __retrieve_sp_dual_solution(
        self,
        is_feasible: bool,
        sp_container: GBDSubproblemContainer,
        dual_soln: Dict[
        ...
        for con_sym, idx_set in sp_container.mixed_comp_con_idx_set.items():
            ...
            # indexed constraint
                ...
                for con_index in idx_set:
                    sp_dual_soln_c[con_index] = retrieve_dual_value(
                        mod_con_sym, con_index
                    )
                dual_soln[dual_mult_sym] = sp_dual_soln_c

    def __set_duality_multipliers(
        self, dual_mult_values: Dict[str, Union[float, Dict[tuple, float]]]
    ):
    ...
    def __print_iteration_output(
        self, iter: int, global_lb: float, global_ub: float, is_feasible: bool
    ):
        message = "Iter: {0:^4} | LB: {1:^15} | UB: {2:^15} | Fbl: {3}".format(
            iter, int(global_lb), int(global_ub), is_feasible
        )
        if self.verbosity == 1:
            print(message)

        self._log_indexed_message(message=message, iter=iter)

    def _print_solver_output(self, solver_output: str):
        if self.verbosity >= 3:
            print(solver_output)

    def __print_solution(
        self,
        is_feasible: bool,
        v_ub: float,
        ...
        message += "\nruntime = {0:.0f} s ({1:.1f} h)".format(
            runtime_seconds, runtime_hours
        )
    ...
    def _log_indexed_message(
        self, message: str, iter: int, sp_sym: str = None, sp_idx: mat.Element = None
    ):

        if sp_sym is not None and sp_idx is not None:
            sp_str = "|sp {0}[{1}]".format(
                sp_sym, "-".join([str(sp_i) for sp_i in sp_idx])
            )
        else:
            sp_str = ""
    ...
    def __generate_entity_sp_index(
        self,
    ...
    def __increment_cut_count(self):
        cut_count = self._engine.get_param_value(self.gbd_problem.cut_count_sym, None)
        self._engine.set_param_value(
            self.gbd_problem.cut_count_sym, None, int(cut_count + 1)
        )
    ...
    def __set_is_feasible_flag(self, flag: bool):
        ...
        cut_count = self.__get_cut_count()
        self._engine.set_param_value(
            self.gbd_problem.is_feasible_sym, (cut_count,), num_flag
        )

    def __set_stored_obj_value(self, value: float):
        cut_count = self.__get_cut_count()
        self._engine.set_param_value(
            self.gbd_problem.stored_obj_sym, (cut_count,), value
        )

    def __update_stored_obj_value(self, added_value: float):
        cut_count = self.__get_cut_count()
        value = self._engine.get_param_value(
            self.gbd_problem.stored_obj_sym, (cut_count,)
        )
        self._engine.set_param_value(
            self.gbd_problem.stored_obj_sym, (cut_count,), value + added_value
        )

    def _get_sp_obj_value(
        self, engine: Engine, obj: mat.MetaObjective, sp_index: mat.Element
    ) -> float:
        obj_idx = self._gbd_problem_builder.generate_entity_sp_index(
            sp_index=sp_index, meta_entity=obj
        )
