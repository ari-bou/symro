...
class GBDProblem(Problem):
    def __init__(
        ...
        super(GBDProblem, self).__init__(
            symbol=None,
        ...
        self.default_primal_sp_sym: str = (
            default_primal_sp_symbol
            if default_primal_sp_symbol is not None
            else DEFAULT_PRIMAL_SP_SYMBOL
        )
        self.default_primal_sp_sym = self.generate_unique_symbol(
            self.default_primal_sp_sym
        )

        self.default_fbl_sp_sym: str = (
            default_fbl_sp_symbol
            if default_fbl_sp_symbol is not None
        ...
        self.primal_sp_objs: Dict[
            str, mat.MetaObjective
        ] = {}  # key: subproblem symbol; value: meta-objective
        self.fbl_sp_objs: Dict[
            str, mat.MetaObjective
        ] = {}  # key: subproblem symbol; value: meta-objective
        ...
        self.std_to_sl_map: Dict[
            str, Tuple[List[mat.MetaVariable], mat.MetaConstraint]
        ] = {}
    ...
    def build_mp_constructs(self, init_lb: float, init_ub: float):

        self.cut_count = eb.build_meta_param(
            problem=self, symbol=self.cut_count_sym, default_value=0
        )
        ...
        ord_set_node = mat.OrderedSetNode(
            start_node=mat.NumericNode(value=1),
            end_node=mat.DeclaredEntityNode(self.cut_count_sym, type=mat.PARAM_TYPE),
        )
