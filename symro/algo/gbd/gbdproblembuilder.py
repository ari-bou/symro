...
from symro.scripting.script import Script, CompoundScript, ScriptType
...
from symro.algo.gbd import GBDProblem, GBDSubproblemContainer
...
class GBDProblemBuilder:
    ...
    def build_and_initialize_gbd_problem(
        self,
        problem: Problem,
            ...
            if isinstance(set_expr, mat.DeclaredSetNode):
                ...
                if not set_expr.is_indexed():
                    ...
                    raise NotImplementedError(
                        "GBD problem builder currently does not support indexed sets"
                ...
                raise NotImplementedError(
                    "GBD problem builder currently does not support undeclared sets"
                    + " as the decomposition axes of a problem"
                )
    ...
    def __collect_complicating_meta_variables(self, comp_var_defs: List[str]):
        for i, comp_var_def in enumerate(comp_var_defs, start=1):
            ...
            meta_vars = self.__retrieve_meta_entity_from_definition(
                problem=self.gbd_problem, entity_def=comp_var_def
            )
            ...
            if len(idx_meta_sets) > 0:
                ...
                if len(sum_idx_meta_sets) > 0:
                    ...
                    sum_idx_set_node = nb.build_idx_set_node(
                        problem=problem, idx_meta_sets=sum_idx_meta_sets
                    )
                    ...
                    if not isinstance(obj_exp_node, mat.ArithmeticExpressionNode):
                        raise ValueError(
                            "GBD problem builder expected an arithmetic expression node"
                ...
                default_dummy_syms = {
                    sym: ms.dummy_element for sym, ms in sp_idx_meta_sets.items()
                }
    ...
    def __categorize_primal_objective(self):
        ...
        for primal_sp in self.gbd_problem.primal_sps:
            ...
            comp_vars, vars = self.__retrieve_complicating_variable_entities(
                meta_obj, comp_meta_vars
            )
    ...
    def __retrieve_complicating_variable_entities(
        self,
        ...
        if not isinstance(expr_node, mat.RelationalOperationNode) and not isinstance(
            expr_node, mat.ArithmeticExpressionNode
        ):
            raise ValueError(
                "GBD problem builder expected a relational operation node or an arithmetic expression node"
        ...
        if meta_entity.idx_set_node is not None:
            ...
            dummy_syms = meta_entity.idx_set_node.get_dummy_element(
                self.gbd_problem.state
            )
        else:
            idx_set = None
            dummy_syms = None
        ...
        entities = expr_node.collect_declared_entities(
            self.gbd_problem.state, idx_set, dummy_syms
        )
        vars = {
            k: v for k, v in entities.items() if isinstance(v, mat.Variable)
    ...
    def build_or_retrieve_defined_primal_sp(
        ...
        if entity_defs is None:
            if sp_sym in self.gbd_problem.subproblems:
                ...
                raise ValueError(
                    "GBD problem builder encountered an undefined subproblem symbol"
                )

        # build new subproblem
        else:

            sp = BaseProblem(symbol=sp_sym, description="Primal subproblem")

            for entity_def in entity_defs:
                self.__add_defined_meta_entity_to_subproblem(
                    problem=self.gbd_problem, subproblem=sp, entity_def=entity_def
                )
    ...
    def __add_defined_meta_entity_to_subproblem(
        self, problem: Problem, subproblem: BaseProblem, entity_def: str
    ):

        # retrieve defined meta-entity or meta-entities
        def_meta_entities = self.__retrieve_meta_entity_from_definition(
            problem=problem, entity_def=entity_def
        )
    ...
    def _add_linked_meta_entities_to_subproblem(
        self,
        problem: Problem,
    ...
    def _build_default_primal_sp(self):
        ...
        if primal_sp_obj_sym is not None:
            primal_sp_meta_obj = self.gbd_problem.meta_objs[
                primal_sp_obj_sym
        ...
        if primal_sp_meta_obj is None:
            raise ValueError(
                "GBD problem builder could not identify a suitable objective function"
    ...
    def __build_slack_variables_and_slackened_constraints(self):
        ...
        meta_cons = list(self.gbd_problem.mixed_comp_cons.values()) + list(
            self.gbd_problem.non_comp_cons.values()
        )
    ...
    def __build_slack_minimization_objective(
        self, obj_sym: str, sl_meta_vars: List[mat.MetaVariable]
    ):

        # formulate infeasibility minimization objective
        fbl_meta_obj = fmr.formulate_slack_min_objective(
            problem=self.gbd_problem,
        ...
        for primal_sp in self.gbd_problem.primal_sps:
            ...
            for meta_con in primal_sp.model_meta_cons:
                if meta_con.symbol in self.gbd_problem.std_to_sl_map:
                    sl_meta_vars, sl_con = self.gbd_problem.std_to_sl_map[
                        meta_con.symbol
                    ]
            ...
            fbl_obj_sym = (
                self.gbd_problem.default_fbl_sp_obj_sym + "_" + primal_sp.symbol
            )
            meta_obj = self.__build_slack_minimization_objective(
                obj_sym=fbl_obj_sym, sl_meta_vars=sl_meta_vars_list
            )

            fbl_sp_sym = primal_sp.symbol + "_FBL"
            fbl_sp = BaseProblem(
                symbol=fbl_sp_sym, description="Feasibility subproblem"
            )
    ...
    def __build_complicating_storage_params(self):
        ...
        for id, meta_var in self.gbd_problem.comp_meta_vars.items():
            if meta_var.symbol not in self.gbd_problem.stored_comp_decisions:
                ...
                storage_symbol = self.gbd_problem.generate_unique_symbol(
                    "{0}_stored".format(meta_var.symbol)
                )
                ...
                self.gbd_problem.stored_comp_decisions[
                    meta_var.symbol
                ] = stored_comp_param
                self.gbd_problem.add_meta_parameter(
                    stored_comp_param, is_auxiliary=True
                )
    ...
    def __build_master_objective(self):
        ...
        eta_node = mat.DeclaredEntityNode(
            symbol=eta.symbol, idx_node=idx_node, type=mat.VAR_TYPE
        )
    ...
    def __build_master_problem_auxiliary_constructs_f(self):
        ...
        for sp in self.gbd_problem.primal_sps:
            ...
            if self.__is_primal_sp_obj_comp[sp_meta_obj.symbol]:
                ...
                node = self.__verify_and_modify_complicating_expression(
                    obj_expr.root_node, idx_set, dummy_syms
                )
        ...
        if len(self.gbd_problem.idx_meta_sets) > 0:
            idx_set_node = nb.build_idx_set_node(
                problem=self.gbd_problem, idx_meta_sets=self.gbd_problem.idx_meta_sets
            )
            ...
            idx_set_node.set_nodes.append(
                mat.IndexingSetNode(dummy_node=dummy_node, set_node=cuts_set_node)
            )
    ...
    def __build_master_problem_auxiliary_constructs_g(self):
        ...
        for name, meta_con in self.gbd_problem.mixed_comp_cons.items():
            ...
            if not isinstance(con_expr.root_node, mat.RelationalOperationNode):
                raise ValueError(
                    "GBD problem builder expected a relational operation node"
                    + " while retrieving the LHS expression of a mixed-complicating constraint"
                    + " '{0}'".format(meta_con)
                )
            rhs_node = self.__verify_and_modify_complicating_expression(
                con_expr.root_node.lhs_operand, idx_set, dummy_syms
            )
            ...
            aux_g_meta_var_sym = self.gbd_problem.generate_unique_symbol(
                "GBD_G_{0}".format(i)
            )
    ...
    def __build_duality_multiplier(self, meta_con: mat.MetaConstraint, id: int):
        ...
        sym = self.gbd_problem.generate_unique_symbol(
            "lambda_{0}".format(meta_con.symbol)
        )
    ...
    def __verify_and_modify_complicating_expression(
        self,
        ...
        node = deepcopy(node)
        self.__verify_complicating_node(
            node, idx_set, dummy_syms
        )  # check whether the problem satisfies property (P)
        comp_node, status = self.__reformulate_node(node, idx_set, dummy_syms)
        sub_node = self.__build_complicating_subtraction_node(
            comp_node, idx_set, dummy_syms
        )
        return sub_node

    def __verify_complicating_node(
        self,
        node: mat.ArithmeticExpressionNode,
        idx_set: mat.IndexingSet,
        ...
        # declared entity
            ...
            if node.symbol not in [
                mv.symbol for mv in self.gbd_problem.comp_meta_vars.values()
            ]:
                return self.PURE_X_NODE

            entities = node.collect_declared_entities(
                self.gbd_problem.state, idx_set, dummy_element
            )
        ...
        # transformation
            ...
            for operand in node.operands:
                statuses.append(
                    self.__verify_complicating_node(operand, idx_set, dummy_element)
                )
            ...
            # Node contains complicating and non-complicating variables
                ...
                if not node.is_reductive():
                    ...
                    raise ValueError(
                        "GBD problem builder encountered a complicating function node"
        ...
        # operation
            ...
            statuses = [
                self.__verify_complicating_node(o, idx_set, dummy_element)
            ...
            if (
                self.PURE_X_NODE in statuses
                and self.PURE_Y_NODE not in statuses
                and self.MIXED_NODE not in statuses
            ):
                return self.PURE_X_NODE

            # all nodes are complicating
            elif (
                self.PURE_X_NODE not in statuses
                and self.PURE_Y_NODE in statuses
                and self.MIXED_NODE not in statuses
            ):
                return self.PURE_Y_NODE
            ...
            # mix of complicating and non-complicating
                ...
                if node.operator in (
                    mat.UNARY_POSITIVE_OPERATOR,
                    mat.UNARY_NEGATION_OPERATOR,
                    mat.ADDITION_OPERATOR,
                    mat.SUBTRACTION_OPERATOR,
                ):
                    return self.MIXED_NODE
                ...
                # non-linearly-separable n-ary operation
                    ...
                    if pure_comp_count == 1 and mixed_comp_count == 0:
                        ...
                        raise ValueError(
                            "GBD problem builder encountered a complicating arithmetic operation node"
                            + " that violates Property P"
                        )
        ...
        # conditional
            ...
            for op_node in node.operands:
                statuses.append(
                    self.__verify_complicating_node(op_node, idx_set, dummy_element)
                )
            return self.__combine_node_statuses(statuses)

        else:
            raise ValueError(
                "GBD problem builder encountered an unexpected node"
    ...
    def __reformulate_node(
        ...
        # declared entity
            ...
            if status == self.PURE_X_NODE:
                ...
                # indexed variable node
                    ...
                    return (
                        self.__reformulate_complicating_entity_node(
                            node, idx_set, dummy_element
                        ),
                        self.PURE_Y_NODE,
                    )
        ...
        # transformation
            ...
            for i, operand in enumerate(node.operands):
                ref_operand, status = self.__reformulate_node(
                    operand, idx_set, dummy_element
                )
        ...
        # operation
            ...
            ref_results = [
                self.__reformulate_node(o, idx_set, dummy_element)
                for o in node.operands
            ]
            ...
            # at least one complicating or mixed-complicating node
                ...
                if (
                    node.operator
                    in (mat.UNARY_POSITIVE_OPERATOR, mat.UNARY_NEGATION_OPERATOR)
                    == 1
                ):
                ...
                # linearly separable
                    ...
                    comp_ref_results = [
                        r for r in ref_results if r[1] in self.COMP_STATUSES
                    ]
        ...
        # conditional
            ...
            for i, op_node in enumerate(node.operands):
                ...
                ref_op_node, status = self.__reformulate_node(
                    op_node, idx_set, dummy_element
                )
    ...
    def __reformulate_complicating_entity_node(
        self,
        ...
        entities = entity_node.collect_declared_entities(
            self.gbd_problem.state, idx_set, dummy_syms
        )
        vars = {k: v for k, v in entities.items() if isinstance(v, mat.Variable)}
        ...
        for var_id, var in vars.items():
            ...
            for (
                comp_meta_var_id,
                comp_meta_var,
            ) in self.gbd_problem.comp_meta_vars.items():
                if comp_meta_var.is_owner(var, self.gbd_problem.state):
                    has_y = True
                    is_y = True
        ...
        sum_idx_set_node = self.__build_idx_set_node_for_mixed_comp_entity_node(
            entity_node, sub_comp_meta_vars
        )
        sum_node = mat.ArithmeticTransformationNode(
            fcn=mat.SUMMATION_FUNCTION,
            idx_set_node=sum_idx_set_node,
    ...
    def __build_idx_set_node_for_mixed_comp_entity_node(
        ...
        for comp_meta_var_id, comp_meta_var in sub_comp_meta_vars.items():
            ...
            def_idx_nodes = comp_meta_var.idx_set_node.get_dummy_component_nodes(
                self.gbd_problem.state
            )
            def_dummy_syms = []
            ...
            for component_set_node in comp_meta_var.idx_set_node.set_nodes:
                ...
                if dim == 1:
                    dim_index += 1
                    challenge_dummy_node = deepcopy(
                        entity_idx_nodes[full_to_reduced_dim_index_map[dim_index]]
                    )
                ...
                # Compound dummy
                    ...
                    for _ in range(dim):
                        ...
                        if is_dim_fixed[dim_index]:
                            ...
                            idx_node = self.__replace_dummy_nodes(
                                idx_node, def_dummy_syms, entity_idx_nodes
                            )
                        else:
                            idx_node = entity_idx_nodes[
                                full_to_reduced_dim_index_map[dim_index]
                            ]
                        component_dummy_nodes.append(deepcopy(idx_node))
                        is_controlled.append(
                            entity_idx_nodes[dim_index].is_controlled()
                        )

                    if any(is_controlled):
                        challenge_dummy_node = mat.CompoundDummyNode(
                ...
                if challenge_dummy_node is not None and challenged_set_node is not None:
                    ...
                    challenged_set_node = self.__replace_dummy_nodes(
                        challenged_set_node, def_dummy_syms, entity_idx_nodes
                    )
                    if not isinstance(challenged_set_node, mat.SetExpressionNode):
                        raise ValueError(
                            "GBD problem builder expected a set expression node"
                            + " while reformulating a mixed entity node"
                        )

                    # Build logical set-membership operation
                    membership_node = mat.SetMembershipOperationNode(
                        operator="in",
            ...
            if comp_meta_var.idx_set_node.constraint_node is not None:
                constraint_node = self.__replace_dummy_nodes(
                    comp_meta_var.idx_set_node.constraint_node,
                    def_dummy_syms,
                    entity_idx_nodes,
                )
            ...
            conjunction_node = mat.LogicalOperationNode(
                operator=mat.CONJUNCTION_OPERATOR, operands=conjunction_operands
            )
        ...
        if len(conjunction_nodes) > 0:
            disjunction_node = mat.LogicalOperationNode(
                operator=mat.DISJUNCTION_OPERATOR, operands=conjunction_nodes
            )
        else:
        ...
        ord_set_node = mat.OrderedSetNode(
            start_node=mat.NumericNode(1), end_node=mat.NumericNode(1)
        )
        sum_idx_set_node = mat.CompoundSetNode(
            set_nodes=[ord_set_node], constraint_node=disjunction_node
        )

        return sum_idx_set_node

    def __replace_dummy_nodes(
        self,
        node: mat.ExpressionNode,
        dummy_syms: List[str],
        replacement_nodes: List[mat.ExpressionNode],
    ) -> mat.ExpressionNode:
        ...
        # Node is not a dummy
            ...
            for child in node.get_children():
                modified_children.append(
                    self.__replace_dummy_nodes(child, dummy_syms, replacement_nodes)
                )
            node.set_children(modified_children)
            return node

    def __build_complicating_subtraction_node(
        self,
        ...
        while not queue.empty():
            ...
            if isinstance(node, mat.DeclaredEntityNode):
                if node.type == mat.VAR_TYPE:
                    ...
                    status = self.__verify_complicating_node(
                        node, idx_set, dummy_element
                    )
                    ...
                    if status in self.COMP_STATUSES:
                        ...
                        cuts_dummy_node = mat.DummyNode(
                            symbol=self.gbd_problem.cuts.dummy_element[0]
                        )
                        if node.idx_node is not None:
                            storage_param_idx_node = deepcopy(node.idx_node)
                            storage_param_idx_node.component_nodes.append(
                                cuts_dummy_node
                            )
                        else:
                            storage_param_idx_node = mat.CompoundDummyNode(
                                component_nodes=[cuts_dummy_node]
                            )

                        # retrieve storage meta-parameter
                        storage_meta_param = self.gbd_problem.stored_comp_decisions[
                            node.symbol
                        ]
        ...
        subtraction_node = mat.SubtractionNode(
            lhs_operand=root_node, rhs_operand=root_node_clone, is_prioritized=True
        )
    ...
    def __build_cuts(self):
        ...
        if (
            self.gbd_problem.is_y_binary
        ):  # build the canonical integer cut if all complicating variables are binary
    ...
    def __build_optimality_cut(self):
        ...
        idx_set_con_node = mat.RelationalOperationNode(
        ...
        # build indexing set node
        idx_set_node = nb.build_idx_set_node(
            problem=self.gbd_problem, idx_meta_sets=idx_meta_sets
        )
        ...
        meta_con = mat.MetaConstraint(
            symbol=self.gbd_problem.opt_cut_con_sym,
            idx_meta_sets=idx_meta_sets,
            idx_set_node=idx_set_node,
            expression=expression,
        )
    ...
    def __build_feasibility_cut(self):
        ...
        # build meta-constraint
        meta_con = mat.MetaConstraint(
            symbol=self.gbd_problem.fbl_cut_con_sym,
            idx_meta_sets=idx_meta_sets,
            idx_set_node=idx_set_node,
            expression=expression,
        )
    ...
    def __build_canonical_integer_cut(self):
        ...
        # build meta-constraint
        meta_con = mat.MetaConstraint(
            symbol=self.gbd_problem.can_int_cut_con_sym,
            idx_meta_sets=idx_meta_sets,
            idx_set_node=idx_set_node,
            expression=expression,
        )
    ...
    def __build_eta_bounding_constraint(self):
        ...
        # build meta-constraint
        meta_con = mat.MetaConstraint(
            symbol=self.gbd_problem.eta_bound_con_sym, expression=expression
        )
    ...
    def __build_optimality_cut_expression(self):
        ...
        stored_obj_node = mat.DeclaredEntityNode(
            symbol=stored_obj.symbol, idx_node=stored_obj_idx_node, type=mat.PARAM_TYPE
        )
        ...
        eta_node = mat.DeclaredEntityNode(
            symbol=eta.symbol, idx_node=eta_idx_node, type=mat.VAR_TYPE
        )
        ...
        # Inequality node
        ineq_op_node = mat.RelationalOperationNode(
    ...
    def __build_aux_f_summation_node(self):
        ...
        # auxiliary variable is indexed with respect to other sets
            ...
            sum_f_node = mat.ArithmeticTransformationNode(
                fcn=mat.SUMMATION_FUNCTION,
                idx_set_node=idx_set_node,
                operands=aux_f_node,
            )
    ...
    def __build_aux_g_summation_node(self, g_id: int):
        ...
        dual_mult_node = mat.DeclaredEntityNode(
        ...
        # auxiliary variable is indexed with respect to other sets
            ...
            idx_set_node.set_nodes.pop()  # Remove CUTS set from indexing set node
            sum_g_node = mat.ArithmeticTransformationNode(
                fcn=mat.SUMMATION_FUNCTION,
                idx_set_node=idx_set_node,
                operands=prod_node,
            )
    ...
    def __build_canonical_integer_cut_expression(self):
        def build_sum_node(
            binary_value: int,
            operand_node_generator: Callable[
                [mat.MetaEntity], mat.ArithmeticExpressionNode
            ],
        ):
            ...
            for _, comp_meta_var in self.gbd_problem.comp_meta_vars.items():
                ...
                storage_meta_param = self.gbd_problem.stored_comp_decisions[
                    comp_meta_var.symbol
                ]

                # Build storage parameter index node
                storage_param_idx_node = nb.build_default_entity_index_node(
                    comp_meta_var
                )
                ...
                storage_param_node = mat.DeclaredEntityNode(
                ...
                if idx_set_con_node is not None:
                    ...
                    con_node = mat.LogicalOperationNode(
                ...
                idx_set_node = nb.build_entity_idx_set_node(
                    problem=self.gbd_problem, meta_entity=comp_meta_var
                )
                ...
                # Build summation node
                var_sum_node = mat.ArithmeticTransformationNode(
                    fcn=mat.SUMMATION_FUNCTION,
                    idx_set_node=idx_set_node,
        ...
        lhs_node = mat.SubtractionNode(
            lhs_operand=sum_nodes[0], rhs_operand=sum_nodes[1]
        )
        ...
        rhs_node = mat.SubtractionNode(lhs_operand=sum_node, rhs_operand=one_node)
    ...
    def __build_mp(self):
        ...
        mp = BaseProblem(
            symbol=self.gbd_problem.mp_symbol, description="Master problem"
        )
    ...
    def __build_subproblem_containers(self):
        ...
        def generate_sp_container(gbd_prob: GBDProblem, sp_idx: mat.Element = None):
            for primal_sp, fbl_sp in zip(gbd_prob.primal_sps, gbd_prob.fbl_sps):
                sp_ctn = GBDSubproblemContainer(
                    primal_sp=primal_sp, fbl_sp=fbl_sp, sp_index=sp_idx
                )
        ...
        for sp_idx_set_dict, sp_container in zip(
            sp_idx_sets, self.gbd_problem.sp_containers
        ):
        ...
        idx_sets = self._assemble_complete_entity_idx_sets_by_symbol(
            mixed_comp_meta_cons
        )
        sp_sym_idx_sets = self._partition_complete_entity_idx_sets_by_sp_sym(
            entity_type=mat.CON_TYPE, complete_idx_sets=idx_sets
        )
        sp_idx_sets = self._partition_sp_entity_idx_sets_by_sp_index(
            sp_entity_idx_sets=sp_sym_idx_sets
        )
    ...
    def _assemble_complete_entity_idx_sets_by_symbol(
        self, meta_entities: List[mat.MetaEntity]
    ):
        ...
        for entity_sym in entity_syms:
            ...
            if self.gbd_problem.get_meta_entity(entity_sym).idx_set_reduced_dim == 0:
                ...
                entity_idx_subsets = (
                    []
                )  # list of indexing subsets for a given entity symbol
                ...
                for me in meta_entities:
                    if me.symbol == entity_sym:
                        ...
                        entity_idx_subset = me.idx_set_node.evaluate(
                            state=self.gbd_problem.state
                        )[0]
                ...
                complete_idx_sets[entity_sym] = mat.OrderedSet.union(
                    *entity_idx_subsets
                )

        return complete_idx_sets

    def _partition_complete_entity_idx_sets_by_sp_sym(
        self, entity_type: str, complete_idx_sets: Dict[str, Optional[mat.OrderedSet]]
    ):

        # instantiate the collection of the indexing sets by subproblem symbol and entity symbol
        sp_entity_idx_sets = (
            {}
        )  # key: primal subproblem symbol; value: dictionary of indexing sets
        ...
        for primal_sp in self.gbd_problem.primal_sps:
            ...
            if entity_type == mat.VAR_TYPE:
                ...
                raise ValueError(
                    "GBD problem builder encountered an unexpected entity type '{0}'".format(
                        entity_type
                    )
            ...
            for me in meta_entities:
                if me.symbol in complete_idx_sets:
                    ...
                    if me.idx_set_reduced_dim == 0:  # scalar entity
                        sp_entity_idx_sets_i[
                            me.symbol
                        ] = None  # assign None for a scalar entity

                    else:  # indexed entity

                        # retrieve the indexing subset of the meta-entity for the current primal subproblem
                        entity_idx_set = me.idx_set_node.evaluate(
                            state=self.gbd_problem.state
                        )[0]

                        # retrieve the indexing set elements that designate relevant entities
                        sp_entity_idx_set = entity_idx_set.intersection(
                            complete_idx_sets[me.symbol]
                        )
    ...
    def _partition_sp_entity_idx_sets_by_sp_index(
        self, sp_entity_idx_sets: Dict[str, Dict[str, Optional[mat.IndexingSet]]]
    ):
        ...
        for sp_container in self.gbd_problem.sp_containers:
            ...
            for entity_sym, sp_entity_idx_set in sp_entity_idx_sets_i.items():
                ...
                if sp_entity_idx_set is None:  # scalar entity
                    ...
                    filter_element = self.generate_entity_sp_index(
                    ...
                    filtered_var_idx_set = mat.filter_set(
                        sp_entity_idx_set, filter_element
                    )
    ...
    def __build_model_scripts(self):
        ...
        ampl_stm.add_included_script_to_compound_script(
            self.gbd_problem.compound_script, ref_mod_script, file_command="model"
        )
        ampl_stm.add_included_script_to_compound_script(
    ...
    def __build_reformulated_model_script(self):
        ...
        return script_builder.generate_problem_model_script(
            problem=self.gbd_problem, model_file_extension=".modr"
        )

    def __build_mp_model_script(self):

        meta_sets_params = [
            self.gbd_problem.cut_count,
        ...
        meta_sets_params.extend(
            [mp for _, mp in self.gbd_problem.stored_comp_decisions.items()]
        )
        meta_sets_params.extend(
            [mp for _, mp in self.gbd_problem.duality_multipliers.items()]
        )
        ...
        script_builder = ScriptBuilder()
        return script_builder.generate_model_script(
            model_file_name=self.gbd_problem.symbol,
    ...
    def __build_problem_declarations(self):
        ...
        for fbl_sp in self.gbd_problem.fbl_sps:
            fbl_sp_decl = script_builder.generate_subproblem_declaration(
                problem=self.gbd_problem, subproblem=fbl_sp, idx_meta_sets=idx_meta_sets
            )
            main_script.statements.append(fbl_sp_decl)

        mp_decl = script_builder.generate_subproblem_declaration(
            problem=self.gbd_problem, subproblem=self.gbd_problem.mp
        )
    ...
    def _generate_sp_idx_set(self):
        ...
        for ims in self.gbd_problem.idx_meta_sets.values():
            ...
            # indexed
                ...
                # TODO: add support for indexed sets as decomposition axes in the GBD problem builder
                raise NotImplementedError(
                    "GBD problem builder does not support indexed sets as decomposition axes"
                )

        return mat.cartesian_product(idx_sets)

    @staticmethod
    def __retrieve_meta_entity_from_definition(problem: Problem, entity_def: str):
    ...
    def generate_entity_sp_index(
        ...
        if entity_index is None:
            entity_index = list(
                meta_entity.idx_set_reduced_dummy_element
            )  # retrieve default entity index
        ...
        for idx_meta_set in self.gbd_problem.idx_meta_sets.values():
            ...
            if meta_entity.is_indexed_with(idx_meta_set):
                ...
                sp_idx_pos += (
                    idx_meta_set.reduced_dim
                )  # update position of the subproblem index

                ent_idx_pos = meta_entity.get_first_reduced_dim_index_of_idx_set(
                    idx_meta_set
                )
                entity_index[
                    ent_idx_pos : ent_idx_pos + idx_meta_set.reduced_dim
                ] = idx_syms  # update entity index
