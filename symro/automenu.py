...
class Dispatcher(Server):
    def __init__(self, action: str, routes: List[str]):
    ...
    def run(self, cmd: Command):
        ...
        if action in self.routes:
            ...
            raise ValueError(
                "Invalid routing symbol encountered: '{0}'.".format(action)
            )
...
class Executor(Server):
    ...
    def add_meta_arg(
        self, name: str, dtype: str, is_optional: bool, default_value: str = None
    ):
        meta_arg = MetaArgument(
            name=name, dtype=dtype, is_optional=is_optional, default_value=default_value
        )
    ...
    def run(self, cmd: Command):
        ...
        if (
            len(args) + len(kwargs) - 1 > len(self.arg_order)
            and not self.can_accept_unspecified_args
        ):
        ...
        for name, arg in kwargs.items():
            if name in self.meta_args:
                ...
                raise ValueError(
                    "Encountered invalid argument identifier '{0}'.".format(name)
                )
