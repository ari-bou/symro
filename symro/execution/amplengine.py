...
class AMPLEngine(Engine):
    ...
    def __init__(self, problem: Problem = None, can_clean_script: bool = False):
    ...
    def setup_ampl_engine(self, problem: Problem, can_clean_script: bool = False):
        ...
        if working_dir_path != "":
            self.api.setOption(name="ampl_include", value=working_dir_path)
    ...
    def __evaluate_ampl_script(self, can_clean_script: bool = False):
        if can_clean_script:
            script_literal = self.__clean_script(
                self.problem.compound_script.main_script
            )
        else:
    ...
    def set_active_problem(
        ...
        Engine.set_active_problem(
            self, problem_symbol=problem_symbol, problem_idx=problem_idx
        )
    ...
    def solve(
        ...
        if problem_symbol is not None:
            self.set_active_problem(
                problem_symbol=problem_symbol, problem_idx=problem_idx
            )
    ...
    def __store_solution(self):
        ...
        for meta_var in p.model_meta_vars:
            ...
            for idx in idx_set:
                ...
                self._store_var(
                    symbol=meta_var.symbol, idx=idx, value=var.value(), lb=lb, ub=ub
                )
        ...
        for meta_obj in p.model_meta_objs:
            ...
            for idx in idx_set:
                ...
                self._store_obj(symbol=meta_obj.symbol, idx=idx, value=obj.value())
        ...
        for meta_con in p.model_meta_cons:
            ...
            for idx in idx_set:
                ...
                self._store_con(
                    symbol=meta_con.symbol, idx=idx, body=body, lb=lb, ub=ub, dual=dual
                )
    ...
    def get_entity(
        self, symbol: str, idx: Union[Tuple, List], etype: str
    ) -> ampl.Entity:
        if etype == "set":
    ...
    def get_entity_instance(
        entity: ampl.Entity, idx: Union[Tuple, List] = None
    ) -> Union[
        ampl.Set, ampl.Parameter, ampl.Variable, ampl.Objective, ampl.Constraint
    ]:
