...
def replace_declared_symbols(
    ...
    while not queue.empty():
        ...
        if isinstance(node, mat.DeclaredEntityNode) or isinstance(
            node, mat.DeclaredSetNode
        ):
...
def retrieve_unbound_symbols(
    root_node: mat.ExpressionNode, in_filter: Iterable[str] = None
) -> Set[str]:
    """
...
def replace_dummy_nodes(
    root_node: mat.ExpressionNode, mapping: Dict[str, mat.ExpressionNode]
) -> mat.ExpressionNode:
    """
...
def map_unbound_symbols_to_controlling_sets(
    set_nodes: List[mat.SetExpressionNode], outer_unb_syms: Iterable[str] = None
            ...
            if isinstance(dummy_node, mat.DummyNode):
                ...
                for component_dummy_node in dummy_node.component_nodes:
                    if isinstance(component_dummy_node, mat.DummyNode):
                        assign_set_node_to_unbound_symbol(
                            component_dummy_node.symbol, set_node
                        )

    return mapping


def generate_unbound_symbol_mapping(
    problem: Problem,
            ...
            if isinstance(node, mat.IndexingSetNode):
                ...
                if isinstance(dummy_node, mat.DummyNode):  # scalar dummy node
                    if dummy_node.symbol not in outer_unb_syms:
                        if (
                            dummy_node.symbol in current_scope_unbound_syms
                            or dummy_node.symbol in blacklisted_unb_syms
                        ):
                            mapping[dummy_node.symbol] = ""
                    else:
                        raise ValueError(
                            "Node builder encountered an indexing set node '{0}'".format(
                                node
                            )
                    ...
                    dummy_node, mat.CompoundDummyNode
                ):  # compound dummy node
                    for component_node in dummy_node.component_nodes:
                        if isinstance(component_node, mat.DummyNode):
                            if component_node.symbol not in outer_unb_syms:
                                if (
                                    component_node.symbol in current_scope_unbound_syms
    ...
    for non_unique_unbound_sym in mapping:
        unique_unbound_sym = problem.generate_unique_symbol(
            base_symbol=non_unique_unbound_sym, symbol_blacklist=defined_symbols
        )
...
def build_default_entity_node(meta_entity: mat.MetaEntity) -> mat.DeclaredEntityNode:
    ...
    return mat.DeclaredEntityNode(
        symbol=meta_entity.symbol, idx_node=entity_index_node, type=meta_entity.type
    )


def build_default_entity_index_node(
    meta_entity: mat.MetaEntity,
) -> Optional[mat.CompoundDummyNode]:
...
def build_entity_idx_set_node(
    problem: Problem,
    meta_entity: mat.MetaEntity,
    remove_sets: Union[List[Union[str, mat.MetaSet]], Dict[str, mat.MetaSet]] = None,
    ...
    return build_idx_set_node(
        problem, idx_meta_sets, meta_entity.idx_set_con_literal, custom_dummy_syms
    )


def build_idx_set_node(
    problem: Problem,
    ...
    if len(custom_unb_syms) > 0:
        ...
        for idx_meta_set in idx_meta_sets:
            ...
            if idx_meta_set.symbol in custom_unb_syms:
                ...
                if (
                    isinstance(custom_dummy_syms_i, int)
                    or isinstance(custom_dummy_syms_i, float)
        ...
        for idx_meta_set in idx_meta_sets:
            ...
            if idx_meta_set.symbol not in custom_unb_syms:
                ...
                for dummy_sym in default_dummy_syms_i:
                    ...
                    if (
                        dummy_sym in all_dummy_syms
                    ):  # check whether the dummy symbol is unique
    ...
    for idx_meta_set in idx_meta_sets:
        ...
        dummy_syms = (
            idx_meta_set.dummy_element
        )  # retrieve the dummy symbols of the component set

        if len(unb_sym_mapping) > 0:
            dummy_syms = [
                unb_sym_mapping.get(d, d) for d in dummy_syms
            ]  # replace selected dummy symbols
        ...
        component_set_node = mat.IndexingSetNode(
            dummy_node=dummy_node, set_node=set_node
        )
    ...
    idx_set_node = mat.CompoundSetNode(
        set_nodes=component_set_nodes, constraint_node=con_node
    )
...
def combine_idx_set_nodes(idx_set_nodes: Iterable[Optional[mat.CompoundSetNode]]):
    ...
    return mat.CompoundSetNode(set_nodes=component_set_nodes, constraint_node=con_node)
...
def build_conjunction_node(operands: List[mat.LogicalExpressionNode]):
    ...
    if len(operands) == 0:
        ...
        return mat.LogicalOperationNode(
            operator=mat.CONJUNCTION_OPERATOR, operands=operands
        )
...
def build_disjunction_node(operands: List[mat.LogicalExpressionNode]):
    ...
    if len(operands) == 0:
        ...
        for op in operands:
            op.is_prioritized = True
        return mat.LogicalOperationNode(
            operator=mat.DISJUNCTION_OPERATOR, operands=operands
        )
...
def build_addition_node(
    terms: List[mat.ArithmeticExpressionNode], is_prioritized: bool = False
):
    return mat.AdditionNode(operands=terms, is_prioritized=is_prioritized)


def build_subtraction_node(
    lhs_term: mat.ArithmeticExpressionNode,
    rhs_term: mat.ArithmeticExpressionNode,
    is_prioritized: bool = False,
):
    return mat.SubtractionNode(
        lhs_operand=lhs_term, rhs_operand=rhs_term, is_prioritized=is_prioritized
    )


def build_multiplication_node(
    factors: List[mat.ArithmeticExpressionNode], is_prioritized: bool = False
):
    return mat.MultiplicationNode(operands=factors, is_prioritized=is_prioritized)


def build_fractional_node(
    ...
    if (
        isinstance(numerator, mat.ArithmeticOperationNode)
        and numerator.operator == mat.DIVISION_OPERATOR
    ):
        num_1 = numerator.get_lhs_operand()
        den_1 = numerator.get_rhs_operand()
    if (
        isinstance(denominator, mat.ArithmeticOperationNode)
        and denominator.operator == mat.DIVISION_OPERATOR
    ):
    ...
    return mat.DivisionNode(
        lhs_operand=num, rhs_operand=den, is_prioritized=is_prioritized
    )


def build_fractional_node_with_unity_numerator(
    denominator: mat.ArithmeticExpressionNode, is_prioritized: bool = True
):
    if not isinstance(denominator, mat.DeclaredEntityNode) and not isinstance(
        denominator, mat.NumericNode
    ):
        denominator.is_prioritized = True
    return build_fractional_node(
        numerator=mat.NumericNode(1),
