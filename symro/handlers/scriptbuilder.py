...
def model_to_ampl(
    problem: Problem, file_name: str = None, working_dir_path: str = None
):

    script_builder = ScriptBuilder()
    script = script_builder.generate_problem_model_script(
        problem=problem, model_file_name=file_name, model_file_extension=None
    )
...
class ScriptBuilder:
    ...
    def generate_problem_model_script(
        self,
        problem: BaseProblem,
        ...
        return self.generate_model_script(
    ...
    def generate_model_script(
        ...
        self._script = scr.Script(
            name=model_file_name, script_type=scr.ScriptType.MODEL
        )
    ...
    @staticmethod
    def generate_subproblem_declaration(
        problem: Problem,
        ...
        prob_node = mat.DeclaredEntityNode(
            symbol=subproblem.symbol, type=mat.PROB_TYPE  # problem symbol
        )
        ...
        decl = ampl_stm.ProblemStatement(
            prob_node=prob_node, idx_set_node=idx_set_node, item_nodes=node_tuples
        )
        return decl
    ...
    def __generate_region_heading(self, name: str):
        comment = ampl_stm.Comment(comment=" {0}".format(name.upper()), is_multi=False)
        self.__add_statement(comment)

        comment = ampl_stm.Comment(comment=" {0}".format("-" * 98), is_multi=False)
        self.__add_statement(comment)

        comment = ampl_stm.Comment(comment=" beginregion", is_multi=False)
        self.__add_statement(comment)

    def __generate_region_footer(self):
        comment = ampl_stm.Comment(comment=" endregion \n\n", is_multi=False)
        self.__add_statement(comment)
