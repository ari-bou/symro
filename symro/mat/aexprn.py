...
from .exprn import LogicalExpressionNode, ArithmeticExpressionNode
from .opern import RelationalOperationNode, ArithmeticOperationNode
from .lexprn import BooleanNode
from .dummyn import CompoundDummyNode
from .setn import CompoundSetNode
...
class ArithmeticTransformationNode(ArithmeticExpressionNode):
    def __init__(
        ...
        operands: Union[
            ArithmeticExpressionNode, List[ArithmeticExpressionNode]
        ] = None,
    ):
    ...
    def __evaluate_reductive_function(
        self, state: State, idx_set: IndexingSet = None, dummy_symbols: Element = None
    ) -> np.ndarray:

        combined_idx_sets = self.combine_indexing_sets(
            state, idx_set, dummy_symbols
        )  # length mp
        ...
        for ip in range(mp):
            x = self.operands[0].evaluate(
                state, combined_idx_sets[ip], combined_dummy_syms
            )
    ...
    def __evaluate_non_reductive_function(
        self, state: State, idx_set: IndexingSet = None, dummy_symbols: Element = None
    ) -> np.ndarray:
    ...
    def to_lambda(
        ...
        if self.is_reductive():
            ...
            combined_idx_set = self.combine_indexing_sets(
            ...
            arg_0 = np.array(
                [
                    self.operands[0].to_lambda(state, idx, combined_dummy_syms)
            ...
            if self.fcn == SUMMATION_FUNCTION:  # reductive summation
                ...
                raise ValueError(
                    "Unable to resolve symbol '{0}'".format(self.fcn)
            ...
            args = np.array(
                [
                    o.to_lambda(state, idx_set_member, dummy_element)
    ...
    def collect_declared_entities(
        ...
        if self.is_reductive():
            ...
            vars = {}
            for ip in range(mp):
                sub_vars = self.operands[0].collect_declared_entities(
                    state, combined_idx_sets[ip], combined_dummy_syms
                )
            ...
            for o in self.operands:
                entities.update(
                    o.collect_declared_entities(state, idx_set, dummy_element)
                )
            return entities

    def combine_indexing_sets(
        self,
        state: State,
        ...
        return self.idx_set_node.generate_combined_idx_sets(  # length mp
            state=state, idx_set=idx_set, dummy_element=dummy_element, can_reduce=False
        )
    ...
    def get_literal(self) -> str:
        ...
        if self.is_reductive():
            literal = "{0} {1} {2}".format(
                AMPL_FUNCTION_SYMBOLS[self.fcn], self.idx_set_node, self.operands[0]
            )
        ...
        # non-reductive transformation
            ...
            if self.fcn == INTEGER_DIVISION_FUNCTION or self.fcn == MODULUS_FUNCTION:
                ...
                literal = "{0} {1} {2}".format(
                    arguments[0], AMPL_FUNCTION_SYMBOLS[self.fcn], arguments[1]
                )

            else:
                arguments = [o.get_literal() for o in self.operands]
                literal = "{0}({1})".format(
                    AMPL_FUNCTION_SYMBOLS[self.fcn], ", ".join(arguments)
                )
...
class ArithmeticConditionalNode(ArithmeticExpressionNode):
        ...
        for ip in range(mp):
            ...
            for k in range(clause_count):
                ...
                if k < clause_count - 1 or not self.has_trailing_else_clause():
                    can_evaluate_operand = self.conditions[k].evaluate(
                        state, sub_idx_set, dummy_element
                    )[0]

                if can_evaluate_operand:
                    y_ip = self.operands[k].evaluate(state, sub_idx_set, dummy_element)[
                        0
                    ]
                    break
    ...
    def collect_declared_entities(
        self, state: State, idx_set: IndexingSet = None, dummy_element: Element = None
    ) -> Dict[tuple, Union[Variable, Parameter]]:
        count_p = 1
        if idx_set is not None:
            count_p = len(idx_set)
        ...
        for ip in range(count_p):
            ...
            for k in range(clause_count):
                ...
                if can_evaluate_operand:
                    entities_ip = self.operands[k].collect_declared_entities(
                        state, sub_idx_set, dummy_element
                    )
                    break
    ...
    def get_children(
        self,
    ) -> List[Union[ArithmeticExpressionNode, LogicalExpressionNode]]:
        children = []
        children.extend(self.operands)
    ...
    def set_children(
        self, operands: List[Union[ArithmeticExpressionNode, LogicalExpressionNode]]
    ):
        count = len(operands)
        if count % 2 == 0:
...
class DeclaredEntityNode(ArithmeticExpressionNode):
    ...
    def evaluate(
        ...
        if self.suffix is None:
            ...
            raise ValueError(
                "Unable to resolve suffix '{0}'".format(self.suffix)
...
class NumericNode(ArithmeticExpressionNode):
    ...
    def __add__(self, other: ArithmeticExpressionNode):
        ...
        if isinstance(other, NumericNode):
            ...
            isinstance(other, ArithmeticOperationNode)
            and other.operator == ADDITION_OPERATOR
        ):
    ...
    def __mul__(self, other: ArithmeticExpressionNode):
        ...
        if isinstance(other, NumericNode):
            ...
            isinstance(other, ArithmeticOperationNode)
            and other.operator == MULTIPLICATION_OPERATOR
        ):
