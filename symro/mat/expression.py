...
def is_linear(root_node: ExpressionNode) -> bool:
    ...
    while not queue.empty():
        ...
        if (
            isinstance(node, ArithmeticOperationNode)
            and node.operator == MULTIPLICATION_OPERATOR
        ):  # multiplication
            ...
            isinstance(node, ArithmeticOperationNode)
            and node.operator == DIVISION_OPERATOR
        ):  # division
            ...
            isinstance(node, ArithmeticOperationNode)
            and node.operator == EXPONENTIATION_OPERATOR
        ):  # exponentiation
...
def __contains_multivariate_reductive_arithmetic_transformation(
    node: ExpressionNode, state: State, idx_set: IndexingSet, dummy_element: Element
) -> bool:

    if (
        isinstance(node, ArithmeticTransformationNode)
        and node.is_reductive()
    ...
    for child in node.get_children():
        if isinstance(child, ArithmeticExpressionNode) or isinstance(
            child, LogicalExpressionNode
        ):
...
class Expression:
    ...
    def to_lambda(self, state: State, idx_set_member: Element = None):

        if idx_set_member is not None:
            if self.indexing_set_node is None:
                raise ValueError(
                    "Non-indexed expression cannot be evaluated at an index"
                )
