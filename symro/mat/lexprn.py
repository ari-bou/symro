...
from .opern import LogicalOperationNode
...
class SetMembershipOperationNode(LogicalExpressionNode):
    ...
    def evaluate(
        ...
        if idx_set is None:
            raise ValueError(
                "Indexing set of a set membership operation cannot be null"
            )

        challenge_elements = self.member_node.evaluate(state, idx_set, dummy_element)
        challenge_elements = [
            tuple([e]) if not isinstance(e, tuple) else e for e in challenge_elements
        ]
...
class SetComparisonOperationNode(LogicalExpressionNode):
    ...
    def evaluate(
        ...
        raise NotImplementedError(
            "evaluate method of Set Comparison Operation Node has not been implemented"
        )
    ...
    def get_literal(self) -> str:
        literal = "{0} {1} {2}".format(
            self.lhs_operand, self.operator, self.rhs_operand
        )
        if self.is_prioritized:
            return "(" + literal + ")"
        return literal
...
class LogicalReductionNode(LogicalExpressionNode):
    ...
    def evaluate(
        ...
        raise NotImplementedError(
            "evaluate method has not yet been implemented for '{0}'".format(type(self))
        )
    ...
    def get_literal(self) -> str:
        literal = (
            AMPL_OPERATOR_SYMBOLS[self.operator]
            + str(self.idx_set_node)
            + str(self.operand)
        )
        if self.is_prioritized:
            literal = "(" + literal + ")"
...
class BooleanNode(LogicalExpressionNode):
    ...
    def __and__(self, other: LogicalExpressionNode):
        ...
        if isinstance(other, BooleanNode):
            ...
            isinstance(other, LogicalOperationNode)
            and other.operator == CONJUNCTION_OPERATOR
        ):
    ...
    def __or__(self, other: LogicalExpressionNode):
        ...
        if isinstance(other, BooleanNode):
            ...
            isinstance(other, LogicalOperationNode)
            and other.operator == DISJUNCTION_OPERATOR
        ):
