...
from .entity import Parameter, Variable
from .exprn import (
    ExpressionNode,
...
class LogicalOperationNode(LogicalExpressionNode):
    def __init__(self, operator: int, operands: List[LogicalExpressionNode] = None):
        ...
        # n-ary operation
            ...
            for i in range(1, len(self.operands)):
                ...
                if self.operator == CONJUNCTION_OPERATOR:  # logical conjunction
                    ...
                    raise ValueError(
                        "Unable to resolve symbol '{0}'".format(self.operator)
    ...
    def to_lambda(
        ...
        args = np.array(
            [o.to_lambda(state, idx_set_member, dummy_element) for o in self.operands]
        )
    ...
    def invert(operand: LogicalExpressionNode):
        return LogicalOperationNode(
            operator=UNARY_INVERSION_OPERATOR, operands=[operand]
        )

    @staticmethod
    def conjunction(
        lhs_operand: LogicalExpressionNode, rhs_operand: LogicalExpressionNode
    ):

        if (
            isinstance(lhs_operand, LogicalOperationNode)
            and lhs_operand.operator == CONJUNCTION_OPERATOR
        ):
            ...
            isinstance(rhs_operand, LogicalOperationNode)
            and rhs_operand.operator == CONJUNCTION_OPERATOR
        ):
            ...
            return LogicalOperationNode(
                operator=CONJUNCTION_OPERATOR, operands=[lhs_operand, rhs_operand]
            )
    ...
    def disjunction(
        ...
        if (
            isinstance(lhs_operand, LogicalOperationNode)
            and lhs_operand.operator == DISJUNCTION_OPERATOR
        ):
            ...
            isinstance(rhs_operand, LogicalOperationNode)
            and rhs_operand.operator == DISJUNCTION_OPERATOR
        ):
            ...
            return LogicalOperationNode(
                operator=DISJUNCTION_OPERATOR, operands=[lhs_operand, rhs_operand]
            )
        ...
        if self.operator == UNARY_INVERSION_OPERATOR:
            ...
            for i, operand in enumerate(self.operands):
                if i == 0:
                    ...
                    s += " {0} {1}".format(
                        AMPL_OPERATOR_SYMBOLS[self.operator], operand
                    )
...
class RelationalOperationNode(LogicalExpressionNode):
    def __init__(
        ...
        operator: int,
        lhs_operand: Union[
            ArithmeticExpressionNode, StringExpressionNode, "RelationalOperationNode"
        ] = None,
        rhs_operand: Union[
            ArithmeticExpressionNode, StringExpressionNode, "RelationalOperationNode"
        ] = None,
        ...
        super().__init__()
        self.operator: int = operator
        self.lhs_operand: Optional[
            Union[
                ArithmeticExpressionNode, StringExpressionNode, RelationalOperationNode
            ]
        ] = lhs_operand
        self.rhs_operand: Optional[
            Union[
                ArithmeticExpressionNode, StringExpressionNode, RelationalOperationNode
            ]
        ] = rhs_operand
    ...
    def evaluate(
        ...
        # less than or equal to
            ...
            raise ValueError(
                "Unable to resolve operator '{0}' as a relational operator".format(
                    self.operator
                )
            )
    ...
    @staticmethod
    def equal(
        lhs_operand: Union[ArithmeticExpressionNode, StringExpressionNode],
        rhs_operand: Union[ArithmeticExpressionNode, StringExpressionNode],
    ):
        return RelationalOperationNode(
            operator=EQUALITY_OPERATOR, lhs_operand=lhs_operand, rhs_operand=rhs_operand
        )
    ...
    def not_equal(
        ...
        return RelationalOperationNode(
            operator=STRICT_INEQUALITY_OPERATOR,
    ...
    def less_than(
        ...
        return RelationalOperationNode(
            operator=LESS_INEQUALITY_OPERATOR,
            lhs_operand=lhs_operand,
            rhs_operand=rhs_operand,
        )
    ...
    def less_than_or_equal(
        ...
        return RelationalOperationNode(
            operator=LESS_EQUAL_INEQUALITY_OPERATOR,
            lhs_operand=lhs_operand,
            rhs_operand=rhs_operand,
        )
    ...
    def greater_than(
        ...
        return RelationalOperationNode(
            operator=GREATER_INEQUALITY_OPERATOR,
            lhs_operand=lhs_operand,
            rhs_operand=rhs_operand,
        )
    ...
    def greater_than_or_equal(
        ...
        return RelationalOperationNode(
            operator=GREATER_EQUAL_INEQUALITY_OPERATOR,
            lhs_operand=lhs_operand,
            rhs_operand=rhs_operand,
        )

    def collect_declared_entities(
        self, state: State, idx_set: IndexingSet = None, dummy_element: Element = None
    ) -> Dict[tuple, Union[Parameter, Variable]]:
        entities = self.lhs_operand.collect_declared_entities(
            state, idx_set, dummy_element
        )
        entities.update(
            self.rhs_operand.collect_declared_entities(state, idx_set, dummy_element)
        )
        return entities
    ...
    def get_literal(self) -> str:
        literal = "{0} {1} {2}".format(
            self.lhs_operand, AMPL_OPERATOR_SYMBOLS[self.operator], self.rhs_operand
        )
        if self.is_prioritized:
            return "(" + literal + ")"
        return literal
...
class SetOperationNode(SetExpressionNode):
    ...
    def evaluate(
        ...
        if self.operator == UNION_OPERATOR:
            ...
            raise ValueError(
                "Unable to resolve operator '{0}' as a set operator".format(
                    self.operator
                )
            )
    ...
    def union(lhs_operand: SetExpressionNode, rhs_operand: SetExpressionNode):
        return SetOperationNode(
            operator=UNION_OPERATOR, lhs_operand=lhs_operand, rhs_operand=rhs_operand
        )
    ...
    def symmetric_difference(
        lhs_operand: SetExpressionNode, rhs_operand: SetExpressionNode
    ):
        return SetOperationNode(
            operator=SYMMETRIC_DIFFERENCE_OPERATOR,
            lhs_operand=lhs_operand,
            rhs_operand=rhs_operand,
        )
...
class ArithmeticOperationNode(ArithmeticExpressionNode):
    ...
    def evaluate(
        ...
        if self.operator in (UNARY_POSITIVE_OPERATOR, UNARY_NEGATION_OPERATOR):
            ...
            if self.operator == UNARY_POSITIVE_OPERATOR:
                ...
                raise ValueError(
                    "Unable to resolve operator '{0}'".format(self.operator)
    ...
    def collect_declared_entities(
        ...
        for operand in self.operands:
            vars.update(
                operand.collect_declared_entities(state, idx_set, dummy_element)
            )
        return vars
    ...
    def negation(operand: ArithmeticExpressionNode):
        ...
        return ArithmeticOperationNode(
            operator=UNARY_NEGATION_OPERATOR, operands=[operand]
        )

    @staticmethod
    def addition(
        lhs_operand: ArithmeticExpressionNode, rhs_operand: ArithmeticExpressionNode
    ):

        if (
            isinstance(lhs_operand, ArithmeticOperationNode)
            and lhs_operand.operator == ADDITION_OPERATOR
        ):
            return rhs_operand + lhs_operand

        elif (
            isinstance(rhs_operand, ArithmeticOperationNode)
            and rhs_operand.operator == ADDITION_OPERATOR
        ):
            ...
            return ArithmeticOperationNode(
                operator=ADDITION_OPERATOR, operands=[lhs_operand, rhs_operand]
            )
    ...
    def multiplication(
        ...
        if (
            isinstance(lhs_operand, ArithmeticOperationNode)
            and lhs_operand.operator == MULTIPLICATION_OPERATOR
        ):
            return rhs_operand * lhs_operand

        elif (
            isinstance(rhs_operand, ArithmeticOperationNode)
            and rhs_operand.operator == MULTIPLICATION_OPERATOR
        ):
            rhs_operand.operands.insert(0, lhs_operand)
            return rhs_operand
            ...
            return ArithmeticOperationNode(
                operator=MULTIPLICATION_OPERATOR, operands=[lhs_operand, rhs_operand]
            )
    ...
    def division(
        ...
        div_op_node = ArithmeticOperationNode(
            operator=DIVISION_OPERATOR, operands=[lhs_operand, rhs_operand]
        )
    ...
    def exponentiation(
        ...
        exp_op_node = ArithmeticOperationNode(
            operator=EXPONENTIATION_OPERATOR, operands=[lhs_operand, rhs_operand]
        )
...
class AdditionNode(ArithmeticOperationNode):
    def __init__(
        self, operands: List[ArithmeticExpressionNode], is_prioritized: bool = False
    ):
        super().__init__(
            operator=ADDITION_OPERATOR, operands=operands, is_prioritized=is_prioritized
        )
...
class StringOperationNode(StringExpressionNode):
    ...
    @staticmethod
    def concatenate(
        lhs_operand: StringExpressionNode, rhs_operand: StringExpressionNode
    ):

        if (
            isinstance(lhs_operand, StringOperationNode)
            and lhs_operand.operator == CONCATENATION_OPERATOR
        ):

            if (
                isinstance(rhs_operand, StringOperationNode)
                and rhs_operand.operator == CONCATENATION_OPERATOR
            ):
            ...
            return StringOperationNode(
                operator=CONCATENATION_OPERATOR, operands=[lhs_operand, rhs_operand]
            )
