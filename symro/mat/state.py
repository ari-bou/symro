...
class State:
    ...
    def __getitem__(self, item: Sequence):
        ...
        if item in self.sets:
            ...
            raise ValueError(
                "Entity with id {0} does not exist in the problem state".format(item)
            )
    ...
    def __setitem__(self, key: Sequence, value: Union[int, float, str, Iterable]):
        ...
        if key in self.sets:
            ...
            raise ValueError(
                "Entity with id {0} does not exist in the problem state".format(key)
            )
    ...
    def get_entity(
        self, symbol: str, idx: Optional[Element], entity_type: str
    ) -> Entity:
        ...
        if entity_type == SET_TYPE:
            ...
            raise ValueError(
                "Unable to resolve '{0}' as an entity type".format(entity_type)
            )
    ...
    def get_set(self, symbol: str, idx: Element = None) -> SSet:
        ...
        if entity is None:
            ...
            raise ValueError(
                "Instance of set {0} does not exist in the state".format(set_literal)
            )
    ...
    def get_parameter(self, symbol: str, idx: Element = None) -> Parameter:
        ...
        if entity is None:
            if idx is None:
                ...
                param_literal = "{0}[{1}]".format(
                    symbol, ",".join([str(i) for i in idx])
                )
            raise ValueError(
                "Instance of parameter {0} does not exist in the state".format(
                    param_literal
                )
            )
    ...
    def add_set(
        ...
        sset = SSet(symbol=symbol, idx=idx, dim=dim, elements=elements)
    ...
    def add_parameter(
        self, symbol: str, idx: Element = None, value: float = 0
    ) -> Parameter:
        param = Parameter(symbol=symbol, idx=idx, value=value)
    ...
    def add_variable(
        ...
        var = Variable(symbol=symbol, idx=idx, value=value, lb=lb, ub=ub)
    ...
    def add_objective(
        ...
        obj = Objective(symbol=symbol, idx=idx, value=value)
    ...
    def add_constraint(
        ...
        con = Constraint(symbol=symbol, idx=idx, value=body, lb=lb, ub=ub, dual=dual)
