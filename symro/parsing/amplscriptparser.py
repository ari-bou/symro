...
import symro.constants as const
...
from symro.parsing.specialcommandparser import SpecialCommandParser
...
class AMPLScriptParser(AMPLParser):
    ...
    def __init__(self, problem: Problem = None, working_dir_path: str = ""):
        super().__init__(problem=problem, working_dir_path=working_dir_path)
    ...
    def __parse_included_script(self, file_name: str, script_type: scr.ScriptType):

        literal = util.read_file(self.working_dir_path, file_name)
        included_script = self._tokenize_included_file(
            literal, script_id=file_name, script_type=script_type
        )
    ...
    def __parse_sentence(
        self,
    ) -> Tuple[bool, Union[ampl_stm.BaseStatement, List[ampl_stm.BaseStatement]]]:
        ...
        can_continue = (
            True  # true if a following sentence needs to be parsed, false otherwise
        )
        ...
        # --- Modelling Commands ---
            ...
            raise NotImplementedError(
                "Parsing logic for environ command not implemented yet"
            )

        elif token in ("solution", "delete", "purge", "redeclare"):
            raise NotImplementedError(
                "Parsing logic for command {0} not yet implemented".format(token)
            )
        ...
        # While/Until Loop
            ...
            raise NotImplementedError(
                "Parsing logic for while/until loop not yet implemented"
            )
        ...
        if not is_compound:
            ...
            if not is_statement:
                ...
                if self._is_last_token():
                    can_continue = (
                        False  # if the current token is last, then terminate parsing
                    )
            ...
            # Statement
                ...
                if not self._next_token():
                    can_continue = (
                        False  # if there is no following token, then terminate parsing
                    )
    ...
    def __parse_obj_declaration(self, direction: str):
        ...
        while self.get_token() != ";":
            ...
            # Suffixes
                ...
                raise NotImplementedError(
                    "Parsing logic for suffix initializations not implemented yet"
                )
    ...
    def __parse_con_declaration(self):
        ...
        meta_con = mat.MetaConstraint(
            symbol=symbol, alias=alias, idx_set_node=idx_set_node, expression=expression
        )
    ...
    # A.13
    def __parse_data_spec(
        self,
    ) -> Union[ampl_stm.IndexedTableDataSpec, ampl_stm.TableDataSpec]:
    ...
    def __parse_indexed_data_spec_form_1(self, idx_set_node: mat.SetExpressionNode):
        ...
        return ampl_stm.IndexedTableDataSpec(
            idx_set_node=idx_set_node, data_spec_components=data_specs, form=form
        )

    # A.13
    def __parse_data_spec_component(
        self,
        data_col_node: Union[
            mat.ArithmeticExpressionNode, mat.StringExpressionNode, mat.BaseDummyNode
        ],
    ):
        data_expr_node = None
        if self.get_token() == "~":
    ...
    def __parse_table_access_statement(self, command: str):
        ...
        return ampl_stm.TableAccessStatement(command=command, table_node=table_node)
    ...
    def __parse_file_command(self):
        ...
        if command == "commands":
            print(
                "AMPL Parser ignored script '{0}'".format(file_name_node)
        ...
        if script_type is not None:
            ...
            self.__parse_included_script(file_name=file_name, script_type=script_type)

        return ampl_stm.FileStatement(command=command, file_name_node=file_name_node)
    ...
    def __parse_option_statement(self):
        ...
        return ampl_stm.OptionStatement(arg_nodes=arg_nodes, redirection=redirection)
    ...
    def __parse_redirection(self):
        ...
        if operator in self.REDIRECTION_OPERATORS:
            ...
            file_name_node = self.__parse_argument()
            return ampl_stm.Redirection(
                operator=operator, file_name_node=file_name_node
            )
        else:
            return None
        ...
        while is_parsing_arg_list:
            ...
            if arg_idx_set_node is not None:
                ...
                if self.get_token() == "(":
                    ...
                    arg_list.append(
                        ampl_stm.DisplayStatement.build_indexed_arg_list(
                            idx_set_node=arg_idx_set_node, arg_nodes=indexed_arg_nodes
                        )
                    )

                # indexing set node is also the argument node
                elif (
                    token == "," or token in self.REDIRECTION_OPERATORS or token == ";"
                ):
                ...
                # indexing set node controls a single argument node
                    ...
                    arg_list.append(
                        ampl_stm.DisplayStatement.build_indexed_arg_list(
                            idx_set_node=arg_idx_set_node, arg_nodes=[arg_node]
                        )
                    )
    ...
    def __parse_io_statement(self):
        ...
        if self.get_token() == "table":
            ...
            raise NotImplementedError(
                "Parsing logic for IO command not yet implemented"
            )
    ...
    def __parse_problem_statement(self):
        ...
        # set problem or declare problem
            ...
            if (
                self.get_token()
                not in ["suffix", ":", ";"] + self.REDIRECTION_OPERATORS
            ):
    ...
    def __parse_data_manipulation_command(self):
        ...
        if command == "let":
            ...
            while self.get_token() != ";":  # terminate at end of statement
                entity_nodes.append(
                    self._parse_declared_entity()
                )  # parse declared entity symbol
                if self.get_token() == ",":
                    self._next_token()  # skip comma
    ...
    def __parse_if_statement_clause(self, is_first_clause: bool = True):
        ...
        clauses.append(
            ampl_stm.IfStatement.build_clause(
        ...
        if self.get_token() == "else":
            ...
            # else
                ...
                clauses.append(
                    ampl_stm.IfStatement.build_clause(
                        operator="else", cdn_expr_node=None, statements=statements
                    )
                )
    ...
    def __parse_for_loop(self):
        ...
        return ampl_stm.ForLoopStatement(
            loop_sym=loop_sym, idx_set_node=idx_set_node, statements=statements
        )
