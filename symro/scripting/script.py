...
class StatementCollection(BaseStatement):
    ...
    def get_validated_literal(
        ...
        for statement in self.statements:
            literals.append(
                "{0}".format(statement.get_validated_literal(indent_level, validator))
            )
        return "\n".join(literals)
...
class Script:
    ...
    def get_validated_literal(
        ...
        literals = []
        for statement in self.statements:
            if validator(statement) and not isinstance(
                statement, SpecialCommandStatement
            ):
                literals.append(
                    "{0}".format(
                        statement.get_validated_literal(indent_level, validator)
                    )
                )
        return "\n".join(literals)
