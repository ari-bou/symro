...
def generate_param_data_statement(
    param_sym: str,
    dim: int,
    values: Dict[Tuple[Union[int, float, str], ...], Union[float, str]],
) -> str:
    declaration = "param {0} := \n".format(param_sym)
    if dim == 0:
...
def __generate_scalar_param_data_block(
    values: Dict[Tuple[Union[int, float, str], ...], Union[float, str]]
) -> str:
...
def __generate_1d_param_data_block(
    values: Dict[Tuple[Union[int, float, str], ...], Union[float, str]]
) -> str:
    data = ""
...
def __generate_2d_param_data_block(
    values: Dict[Tuple[Union[int, float, str], ...], Union[float, str]],
    row_dim_pos: int,
    col_dim_pos: int,
) -> str:
...
def __generate_nd_param_data_block(
    values: Dict[Tuple[Union[int, float, str], ...], Union[float, str]],
    row_dim_pos: int,
    col_dim_pos: int,
) -> str:
    ...
    for multi_index, value in values.items():
        fixed_multi_index = __replace_floating_indices_with_placeholders(
            multi_index, floating_index_positions
        )
    ...
    for raw_fixed_multi_index, values in value_maps.items():
        data_2d = __generate_2d_param_data_block(
            values=values, row_dim_pos=row_dim_pos, col_dim_pos=col_dim_pos
        )
        fixed_multi_index = []
...
def __replace_floating_indices_with_placeholders(
    multi_index: tuple, floating_index_positions: List[int]
) -> tuple:
